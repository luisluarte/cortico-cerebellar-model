...
wsls_obj <- function(params_matrix) {
  ...
  results <- foreach(ind = 1:num_ind, .combine = cbind, .export=c("lag_Reward", "lag_Ch", "rnn_p_logits", "p_val", "rnn_mu")) %dopar% {
...
q_learning_obj <- function(params_matrix) {
  ...
  results <- foreach(ind = 1:num_ind, .combine = cbind, .export=c("N_trials", "stan_data", "lag_Resp", "lag_Ch", "rnn_p_logits", "p_val", "rnn_mu")) %dopar% {
...
cortico_obj <- function(params_matrix) {
  ...
  results <- foreach(ind = 1:num_ind, .combine = cbind, .export=c("N_trials", "X", "W_gen", "W_ach1", "W_ach2", "W_thal", "Pi_vec", "ITI", "rnn_p_logits", "p_val", "rnn_mu")) %dopar% {
