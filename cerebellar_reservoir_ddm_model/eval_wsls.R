...
# Extract RNN predictions (We can just use the BCE formula by loading predictions if we saved them, or we can quickly re-infer)
# To be fast, I'll just load the p_val from the Q-learning script output if I can, but I'll recreate it here.
library(torch)
...
# WSLS Logic with 10% Lapse Rate to avoid catastrophic BCE penalties
epsilon <- 0.1
wsls_p <- ifelse(lag_Reward > 0, epsilon, 1 - epsilon)
wsls_p[lag_Reward == 0 & lag_Ch == 0] <- 0.5 # First trial neutral

wsls_bce <- -mean(p_val * log(wsls_p) + (1 - p_val) * log(1 - wsls_p))
cat(sprintf("WSLS BCE: %.4f\n", wsls_bce))
