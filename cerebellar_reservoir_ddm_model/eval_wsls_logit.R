...
# Logit Fit for Empirical WSLS
df <- data.frame(Switch = Switch, lag_Reward = lag_Reward)
mod <- glm(Switch ~ lag_Reward, data = df, family = binomial())
wsls_p <- predict(mod, type="response")

# Clamp to prevent infinites
wsls_p[wsls_p < 1e-7] <- 1e-7
wsls_p[wsls_p > 1 - 1e-7] <- 1 - 1e-7

wsls_bce <- -mean(p_val * log(wsls_p) + (1 - p_val) * log(1 - wsls_p))

cat(sprintf("WSLS (Logit Fit) BCE: %.4f\n", wsls_bce))
cat(sprintf("Intercept (Beta 0): %.4f\n", coef(mod)[1]))
cat(sprintf("Lag Reward (Beta 1): %.4f\n", coef(mod)[2]))
