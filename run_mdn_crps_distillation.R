...
cat("Training N100 MDN (K=2, L=1) with CRPS Metric Tracking...\n")
...
with_no_grad({
  for (s in val_subjs) {
    ...
    samp_S1 <- ifelse(comp1_S1 == 1, 
                      rlnorm(N_v * M, mu1_M, sig1_M) + tau1_M,
                      rlnorm(N_v * M, mu2_M, sig2_M) + tau2_M)
    samp_S2 <- ifelse(comp1_S2 == 1, 
                      rlnorm(N_v * M, mu1_M, sig1_M) + tau1_M,
                      rlnorm(N_v * M, mu2_M, sig2_M) + tau2_M)
...
# Compute CRPS via Energy Score Approximation
...
with_no_grad({
  for (s in 1:N_subj) {
    ...
    V1 <- (exp(sig_arr[,1]^2) - 1) * exp(2*mu_arr[,1] + sig_arr[,1]^2)
    V2 <- (exp(sig_arr[,2]^2) - 1) * exp(2*mu_arr[,2] + sig_arr[,2]^2)
    
    mean_t <- pi_arr[,1] * E1 + pi_arr[,2] * E2
    var_t <- pi_arr[,1] * (V1 + E1^2) + pi_arr[,2] * (V2 + E2^2) - mean_t^2
    
    soft_expected_rt <- c(soft_expected_rt, mean_t)
    soft_var_rt <- c(soft_var_rt, var_t)
...
stan_data$soft_expected_rt <- soft_expected_rt
stan_data$soft_var_rt <- soft_var_rt
stan_data$H <- H
