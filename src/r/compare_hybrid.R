...
cat("Setting up data for Hybrid Comparison models...\n")
...
cat("Re-Running Biological C++ MCMC (Constrained Distillation Prior)...\n")
set.seed(42)
...
sourceCpp("hybrid_mcmc.cpp")
# Only 250 iterations needed for Bio since it's already constrained at optimum
bio_res <- run_empirical_mcmc_constrained(250, length(test_subjs), data_list$subj_indices, data_list$X, data_list$ITI, 
                                 data_list$y_choice, data_list$y_rt, Pi_mat,
                                 W_gen, W_ach1, W_ach2, W_thal, data_list$N_total)

# Burn-in: discard first 250 iterations for Q-Learn/WSLS 
burn <- 251:500
...
cat("\n--- HYBRID MODEL COMPARISON (Bio-Distilled vs Baseline-Empirical) ---\n")
