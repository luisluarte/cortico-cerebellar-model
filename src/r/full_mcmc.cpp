...
mat simulate_bio_subject(int subj_id, int N_trials, mat X, vec ITI, 
  ...
  for(int t = 0; t < N_trials; t++) {
    ...
    // Safety check to prevent math explosions
...
LL_Result eval_subject_likelihood(mat mu_hist, vec y_choice, mat y_rt, vec y_sigma) {
  LL_Result res;
  ...
  mat W_mu     = XX_inv * (mu_hist.t() * y_rt);
  
  vec bio_logits = mu_hist * W_policy;
  mat bio_mu     = mu_hist * W_mu;
  
  vec p = 1.0 / (1.0 + exp(-bio_logits));
  vec bce = y_choice % log(p + 1e-8) + (1.0 - y_choice) % log(1.0 - p + 1e-8);
  
  vec rt_ll = zeros<vec>(N);
  for(int t = 0; t < N; t++) {
    double sq_err = mean(square(y_rt.row(t) - bio_mu.row(t)));
    rt_ll(t) = - sq_err / (2.0 * std::pow(y_sigma(t), 2));
  }
...
// [[Rcpp::export]]
List run_hierarchical_mcmc(int iters, int N_subjs, uvec subj_indices, 
                           mat X, vec ITI, vec y_choice, mat y_rt, vec y_sigma, mat Pi_mat,
  ...
  for(int iter = 0; iter < iters; iter++) {
    ...
    for(int s = 0; s < N_subjs; s++) {
      ...
      ll_pop_old += eval_subject_likelihood(hist_old, y_choice.elem(idx), y_rt.rows(idx), y_sigma.elem(idx)).total_ll;
      ...
      ll_pop_new += eval_subject_likelihood(hist_new, y_choice.elem(idx), y_rt.rows(idx), y_sigma.elem(idx)).total_ll;
    }
    
    double prior_mu_old = accu(-0.5 * square(mu_pop - mu_EB));
    double prior_mu_new = accu(-0.5 * square(mu_pop_new - mu_EB));
    ...
    for(int s = 0; s < N_subjs; s++) {
      ...
      LL_Result res_old = eval_subject_likelihood(
        ...
        y_choice.elem(idx), y_rt.rows(idx), y_sigma.elem(idx)
      );
      
      vec params_new = inv_logit_scaled(mu_pop + exp(sigma_pop) % Z_new);
      LL_Result res_new = eval_subject_likelihood(
    ...
    trace_sigma.row(iter) = sigma_pop.t();
    log_lik_trace.row(iter) = current_iter_ll.t();
    
    if(iter % 10 == 0) Rcpp::Rcout << "Iteration " << iter << "\n";
  }
  
  return List::create(Named("trace_mu") = trace_mu, 
                      Named("trace_sigma") = trace_sigma, 
                      Named("log_lik_trace") = log_lik_trace);
}
