...
rnn_phase1_subjs <- readRDS("rnn_oob_subjects.rds")
...
X_lag_Ch <- numeric(N_trials)
for (i in 1:N_trials) {
    if (lag_Ch[i] > 0) {
        X_lag_Ch[i] <- (lag_Ch[i] - min_bd) / (max_bd - min_bd)
    } else {
        X_lag_Ch[i] <- 0.0 # Default if no lag
    }
}
...
with_no_grad({
    for (s in rnn_phase1_subjs) {
        ...
        all_ITI <- c(all_ITI, stan_data$ITI[idx][mask])
    }
})
...
saveRDS(targets, "distillation_targets_v2_oob.rds")
cat("Saved to distillation_targets_v2_oob.rds\n")
