...
res <- mclapply(1:nrow(tasks), function(i) {
    s <- tasks$subj_id[i]
    s_idx <- which(subj_indices == s)
    out <- run_bio_mcmc_chain(10, X[s_idx,], ITI[s_idx], reward[s_idx], choices[s_idx], y_rt[s_idx], rnn_mu[s_idx,], "empirical", "flat", mu_EB, W_gen, W_ach1, W_ach2, W_thal, Pi_mat[s_idx,])
    return(out)
}, mc.cores=4)
