...
cat("Loading Data (N=100)...\n")
targets <- readRDS("distillation_targets_N100.rds")
...
pilot_subjs <- unique_subjs # N=100
...
cat("Running Stan Bio Model (Pooled, N=100)...\n")
...
cat("Running Stan QLearn Model (Pooled, N=100)...\n")
...
cat("Running Stan WSLS Model (Pooled, N=100)...\n")
...
cat("\n===========================\nLOO COMPARISON (N=100, Pooled)\n===========================\n")
print(comp)

saveRDS(list(Bio = loo_bio, WSLS = loo_wsls, QLearn = loo_qlearn, comp = comp), "final_elpd_n100.rds")
