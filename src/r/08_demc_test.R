...
cat("Starting 08_demc_test.R (bio_dist, Differential Evolution MCMC, 8 Chains, 2000 iters)...\n")
...
sourceCpp("bio_mcmc_demc.cpp")

tasks <- 0:(length(subjs)-1)
n_cores <- 24
iters <- 2000
num_chains <- 8 # using 8 chains for DE-MC to ensure perfect scale coverage
burn <- 1001:2000

res <- mclapply(1:length(tasks), function(i) {
  s <- tasks[i]
  s_idx <- which(subj_indices == s)
  out_chains <- run_bio_demc(iters, num_chains, X[s_idx,], ITI[s_idx], reward[s_idx], choices[s_idx], y_rt[s_idx], rnn_mu[s_idx,], "distillation", "locked", mu_EB, W_gen, W_ach1, W_ach2, W_thal, Pi_mat[s_idx,])
  return(out_chains)
}, mc.cores=n_cores)
...
for(s in 0:(length(subjs)-1)) {
  ...
  arr <- array(0, dim=c(length(burn), num_chains, num_params))
...
total_ll_trace <- matrix(0, length(burn)*num_chains, nrow(d))
trace_row <- 1
for(c in 1:num_chains) {
  for(iter in burn) {
    for(s in 0:(length(subjs)-1)) {
