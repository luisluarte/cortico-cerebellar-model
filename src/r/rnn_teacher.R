...
cat("##########################################################################")
cat("\nparticipant selecion for phase 1 libs...\n")
cat("##########################################################################\n")
...
# extract 50 participants for RNN phase 1
...
cat("\n#### final LNSO cv metrics ####\n")
cat(sprintf("mean roc-auc: %.4f (%.4f)\n", mean(cv_auc), sd(cv_auc)))
cat(sprintf("mean pr-auc: %.4f (%.4f)\n", mean(cv_pr_auc), sd(cv_pr_auc)))
cat(sprintf("mean crps: %.4f (%.4f)\n", mean(cv_crps), sd(cv_crps)))
cat(sprintf("mean acc: %.4f (%.4f)\n", mean(cv_acc), sd(cv_acc)))

# retrain the final frozen model on all 50 participants
cat("\nretraining final froz model on all 50 isolated participants...\n")
