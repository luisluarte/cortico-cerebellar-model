...
cat("Compiling Full Hierarchical Bio Stan Model with BPTT...\n")
mod_full <- cmdstan_model("full_bio_bptt.stan", 
                          stanc_options = list("allow-undefined"=TRUE), cpp_options = list(USER_HEADER = file.path(getwd(), "bio_bptt.hpp")))

cat("Running Stan BPTT timing test (N=3)...\n")
