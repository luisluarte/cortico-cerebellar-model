...
cat("Setting up data for OOB models...\n")
targets <- readRDS("distillation_targets_v2_oob.rds")
unique_subjs <- unique(targets$subjs)

# Select Out-Of-Bag subjects 51 to 100
test_subjs <- unique_subjs
...
eval_q <- function(params, idx, d) {
  ...
  for(t in 1:N) {
    ...
    unchosen <- ifelse(d$lag_resp[idx[t]] == 1, 2, 1)
    if(d$lag_reward[idx[t]] > 0) {
  ...
  rt_ll <- numeric(N)
  for(t in 1:N) {
    sq_err <- mean((d$y_rt[idx[t],] - bio_mu[t,])^2)
    rt_ll[t] <- - sq_err / (2.0 * d$y_sigma[idx[t]]^2)
  }
  return(bce + rt_ll)
}
...
run_mcmc <- function(eval_fn, N_params, d, iters=100) {
  N_subjs <- length(unique(d$subj_indices))
  mu_pop <- rep(0, N_params)
  sigma_pop <- rep(0, N_params)
  ...
  for(iter in 1:iters) {
    ...
    if (log(runif(1)) < (ll_new - ll_old)) mu_pop <- mu_pop_new
...
cat("Running Q-Learning MCMC...\n")
q_trace <- run_mcmc(eval_q, 3, data_list, 100)
cat("Running WSLS MCMC...\n")
wsls_trace <- run_mcmc(eval_wsls, 2, data_list, 100)

cat("Re-Running Biological C++ MCMC on Held-Out Subjects...\n")
set.seed(42)
...
for(s in 0:(length(test_subjs)-1)) {
  ...
  subj_var <- apply(as.matrix(data_list$X[s_idx, ]), 2, var)
...
bio_res <- run_hierarchical_mcmc(100, length(test_subjs), data_list$subj_indices, data_list$X, data_list$ITI, 
                                 data_list$y_choice, data_list$y_rt, data_list$y_sigma, Pi_mat,
                                 W_gen, W_ach1, W_ach2, W_thal, data_list$N_total)

bio_trace <- bio_res$log_lik_trace

cat("Computing PSIS-LOO and Comparing...\n")
loo_q <- loo(q_trace)
loo_wsls <- loo(wsls_trace)
loo_bio <- loo(bio_trace)

cat("\n--- OOB MODEL COMPARISON ---\n")
