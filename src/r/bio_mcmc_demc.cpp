...
// [[Rcpp::export]]
List run_bio_demc(int iters, int num_chains, NumericMatrix X_r, NumericVector ITI_r, NumericVector reward_r,
  ...
  mat Z_chains = randn<mat>(num_chains, d) * 0.01;
  vec ll_chains = zeros<vec>(num_chains);
  vec step_sizes = zeros<vec>(num_chains); step_sizes.fill(0.05);
  
  for(int c=0; c<num_chains; c++) {
      vec params = inv_logit_scaled(mu_pop + exp(sigma_pop) % Z_chains.row(c).t());
      mat bio_mu = simulate_bio_subject(N_trials, X, ITI, reward, W_gen, W_ach1, W_ach2, W_thal, Pi_mat, params);
      ll_chains(c) = target_mode == "distillation" ? eval_distillation(bio_mu, rnn_mu) : eval_empirical(bio_mu, human_choices, y_rt);
  }
  
  cube all_trace_z(iters, d, num_chains);
  cube all_trace_ll(iters, N_trials, num_chains);
  
  double gamma = 2.38 / sqrt(2.0 * d);
  int burn_in = iters / 2;
  ...
  for(int iter = 0; iter < iters; iter++) {
      for(int i = 0; i < num_chains; i++) {
          vec Z_new;
          if (iter < burn_in) {
              // Phase 1: Independent Adaptive Spherical RWMH (let them find the local mode)
              Z_new = Z_chains.row(i).t() + randn<vec>(d) * step_sizes(i);
          } else {
              // Phase 2: Differential Evolution MCMC (DE-MC)
              int j = i, k = i;
              while(j == i) j = rand() % num_chains;
              while(k == i || k == j) k = rand() % num_chains;
              
              double g = (randu() < 0.1) ? 0.98 : gamma; // 10% chance snooker jump
              vec diff = Z_chains.row(j).t() - Z_chains.row(k).t();
              Z_new = Z_chains.row(i).t() + g * diff + randn<vec>(d) * 1e-4; // small ergodic noise
          }
          ...
          double prior_old = accu(-0.5 * square(Z_chains.row(i).t()));
          double prior_new = accu(-0.5 * square(Z_new));
          
          double log_alpha = (ll_new + prior_new) - (ll_chains(i) + prior_old);
          if(std::isnan(log_alpha)) log_alpha = -1e9;
          
          double acc_prob = exp(log_alpha);
          if(acc_prob > 1.0) acc_prob = 1.0;
          
          if(log(randu()) < log_alpha) {
              Z_chains.row(i) = Z_new.t();
              ll_chains(i) = ll_new;
          }
          
          if(iter < burn_in) {
              double eta = 1.0 / sqrt(iter + 1.0);
              step_sizes(i) = exp(log(step_sizes(i)) + eta * (acc_prob - 0.234));
              if(step_sizes(i) > 5.0) step_sizes(i) = 5.0;
              if(step_sizes(i) < 1e-4) step_sizes(i) = 1e-4;
          }
          
          all_trace_z.slice(i).row(iter) = Z_chains.row(i);
          all_trace_ll.slice(i).row(iter).fill(ll_chains(i) / N_trials);
      }
  }
  
  List out_chains(num_chains);
  for(int c=0; c<num_chains; c++) {
