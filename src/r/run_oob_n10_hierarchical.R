...
cat("Loading N=100 Distilled Data & isolating 10 OOB subjects...\n")
...
for(i in 1:N) {
  if (stan_data$subj[i] != current_subj) {
    ...
    # FIXED: lag_Resp should be 1 if they chose Bd1 previously, 0 if Bd2
...
# Get ALL out-of-bag subjects
oob_subjs <- unique(stan_data$subj[!(stan_data$subj %in% train_subjs)])

# Sample exactly 10
set.seed(123)
selected_10_oob <- sample(oob_subjs, 10)

cat(sprintf("Selected 10 OOB Participants: %s\n", paste(selected_10_oob, collapse=", ")))

valid_idx <- which(lag_Resp >= 0 & stan_data$subj %in% selected_10_oob & lag_Ch > 0)
...
# Remap subj_final to 1..10
subj_mapped <- numeric(N_final)
for (k in 1:10) {
  subj_mapped[subj_final == selected_10_oob[k]] <- k
}
...
cat("Running Stan Bio Model (Hierarchical, OOB N=10)...\n")
data_bio <- list(N = N_final, K = 10, D = 32, subj_id = subj_mapped, y = y_final, trace = mu_hist)
fit_bio <- mod_readout$sample(data = data_bio, chains = 4, parallel_chains = 4, iter_warmup = 1000, iter_sampling = 1000, refresh=500)
loo_bio <- fit_bio$loo()
cat("Bio ELPD:\n")
print(loo_bio)

cat("Running Stan QLearn Model (Hierarchical, OOB N=10)...\n")
data_qlearn <- list(N = N_final, K = 10, D = 8, subj_id = subj_mapped, y = y_final, trace = q_hist)
...
cat("Running Stan WSLS Model (Hierarchical, OOB N=10)...\n")
data_wsls <- list(N = N_final, K = 10, subj_id = subj_mapped, y = y_final,
...
cat("\n===========================\nLOO COMPARISON (OOB N=10, Hierarchical)\n===========================\n")
print(comp)

saveRDS(list(Bio = loo_bio, WSLS = loo_wsls, QLearn = loo_qlearn, comp = comp), "final_elpd_oob_hierarchical_n10.rds")
