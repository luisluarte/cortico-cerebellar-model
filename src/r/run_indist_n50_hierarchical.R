...
cat("Loading N=100 Distilled Data & isolating ALL 50 In-Distribution subjects...\n")
...
# Select EXACTLY the 50 IN-DISTRIBUTION subjects
valid_idx <- which(lag_Resp >= 0 & stan_data$subj %in% train_subjs & lag_Ch > 0)
unique_indist <- unique(stan_data$subj[valid_idx])

cat(sprintf("Retained %d trials across %d In-Distribution participants.\n", length(valid_idx), length(unique_indist)))
...
# Remap subj_final to 1..50
subj_mapped <- numeric(N_final)
for (k in 1:length(unique_indist)) {
  subj_mapped[subj_final == unique_indist[k]] <- k
}
...
cat("Loading Stan Hierarchical Models...\n")
mod_readout <- cmdstan_model("readout.stan")
mod_wsls <- cmdstan_model("wsls.stan")

cat("Running Stan Bio Model (Hierarchical, In-Dist N=50)...\n")
data_bio <- list(N = N_final, K = 50, D = 32, subj_id = subj_mapped, y = y_switch_final, trace = mu_hist)
fit_bio <- mod_readout$sample(data = data_bio, chains = 4, parallel_chains = 4, iter_warmup = 1000, iter_sampling = 1000, refresh=500)
loo_bio <- fit_bio$loo()
cat("Bio ELPD:\n")
print(loo_bio)

cat("Running Stan QLearn Model (Hierarchical, In-Dist N=50)...\n")
data_qlearn <- list(N = N_final, K = 50, D = 8, subj_id = subj_mapped, y = y_switch_final, trace = q_hist)
...
cat("Running Stan WSLS Model (Hierarchical, In-Dist N=50)...\n")
data_wsls <- list(N = N_final, K = 50, subj_id = subj_mapped, y = y_choice_final,
                  win = ifelse(lag_R_final > 0, 1, 0),
                  lag_c = lag_Resp_final,
...
cat("\n===========================\nLOO COMPARISON (In-Dist N=50, Hierarchical)\n===========================\n")
print(comp)

saveRDS(list(Bio = loo_bio, WSLS = loo_wsls, QLearn = loo_qlearn, comp = comp), "final_elpd_indist_hierarchical_n50.rds")
