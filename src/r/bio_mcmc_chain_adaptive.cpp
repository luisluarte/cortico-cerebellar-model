...
List run_bio_mcmc_chain(int iters, NumericMatrix X_r, NumericVector ITI_r, NumericVector reward_r,
  ...
  // Starting point: initialize almost EXACTLY on the NSGA-II prior to prevent getting stuck
  ...
  return List::create(Named("trace_z") = trace_z, Named("trace_ll") = trace_ll, Named("final_step") = step_size);
}
