...
# Sample exactly 5 IN-DISTRIBUTION subjects
set.seed(999)
selected_5_indist <- sample(train_subjs, 5)

cat(sprintf("Selected 5 In-Distribution Participants: %s\n", paste(selected_5_indist, collapse=", ")))
...
cat("Running Stan Bio Model (Hierarchical, In-Dist N=5)...\n")
...
cat("Running Stan QLearn Model (Hierarchical, In-Dist N=5)...\n")
...
cat("Running Stan WSLS Model (Hierarchical, In-Dist N=5)...\n")
data_wsls <- list(N = N_final, K = 5, subj_id = subj_mapped, y = y_choice_final,
...
cat("\n===========================\nLOO COMPARISON (In-Dist N=5, Hierarchical)\n===========================\n")
print(comp)

saveRDS(list(Bio = loo_bio, WSLS = loo_wsls, QLearn = loo_qlearn, comp = comp), "final_elpd_indist_hierarchical_n5.rds")
