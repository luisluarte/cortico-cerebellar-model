...
cat("Loading N=100 Distilled Data & Training IDs...\n")
...
# ISOLATE OUT-OF-BAG SUBJECTS ONLY
valid_idx <- which(lag_Resp == 1 & !(stan_data$subj %in% train_subjs))
unique_oob <- unique(stan_data$subj[valid_idx])

cat(sprintf("Retained %d OOB trials across %d unseen participants.\n", length(valid_idx), length(unique_oob)))
...
cat("Running Stan Bio Model (Pooled, OOB N=50)...\n")
...
cat("Running Stan QLearn Model (Pooled, OOB N=50)...\n")
...
cat("Running Stan WSLS Model (Pooled, OOB N=50)...\n")
...
cat("\n===========================\nLOO COMPARISON (OOB N=50, Pooled)\n===========================\n")
print(comp)

saveRDS(list(Bio = loo_bio, WSLS = loo_wsls, QLearn = loo_qlearn, comp = comp), "final_elpd_oob.rds")
