...
// [[Rcpp::export]]
List run_empirical_mcmc_constrained(int iters, int N_subjs, uvec subj_indices, 
                           mat X, vec ITI, vec y_choice, vec y_rt, mat Pi_mat,
                           mat W_gen, mat W_ach1, mat W_ach2, mat W_thal, int N_total_trials) {
  
  ITI = ITI / max(ITI);
  // PERFECT DISTILLATION PARAMETERS
  vec mu_EB = {-0.491991, 0.195432, -6.754977, -3.496445, 0.075258, -0.982694, -2.459656};
  vec mu_pop = mu_EB; 
  vec sigma_pop = zeros<vec>(7); sigma_pop.fill(-3.0); // TIGHT VARIANCE TO KEEP IT ON MANIFOLD
