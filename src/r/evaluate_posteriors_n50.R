...
data_bio <- list(N = N_final, K = K_final, D = 32, subj_id = subj_mapped, y = y_switch_final, trace = mu_hist)
...
data_qlearn <- list(N = N_final, K = K_final, D = 8, subj_id = subj_mapped, y = y_switch_final, trace = q_hist)
...
data_wsls <- list(N = N_final, K = K_final, subj_id = subj_mapped, y = y_choice_final,
...
cat("\n=========================================\n")
cat("REACTION TIME (RT) PREDICTIVE EVALUATION (N=50)\n")
cat("=========================================\n")
...
saveRDS(list(
  comp = comp,
  pr_auc = list(bio = bio_pr_auc, qlearn = qlearn_pr_auc, wsls = wsls_pr_auc),
  brier = list(bio = bio_brier, qlearn = qlearn_brier, wsls = wsls_brier),
  rt_cor = list(bio = cor(bio_certainty, RT_final), qlearn = cor(qlearn_certainty, RT_final), wsls = cor(wsls_certainty, RT_final)),
  rt_rmse = list(base = rmse_baseline, bio = rmse_bio, qlearn = rmse_q)
), "final_eval_n50_results.rds")
