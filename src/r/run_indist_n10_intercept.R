...
cat("Loading N=100 Distilled Data & isolating 10 In-Distribution subjects...\n")
...
# Sample exactly 10 IN-DISTRIBUTION subjects
set.seed(42)
selected_10_indist <- sample(train_subjs, 10)

cat(sprintf("Selected 10 In-Distribution Participants: %s\n", paste(selected_10_indist, collapse=", ")))
...
for (k in 1:10) {
  subj_mapped[subj_final == selected_10_indist[k]] <- k
}
...
cat("Loading Stan Hierarchical INTERCEPT Models...\n")
mod_readout <- cmdstan_model("readout_intercept.stan")
mod_wsls <- cmdstan_model("wsls.stan")

cat("Running Stan Bio Model (Hierarchical + Intercept, In-Dist N=10)...\n")
data_bio <- list(N = N_final, K = 10, D = 32, subj_id = subj_mapped, y = y_switch_final, trace = mu_hist)
fit_bio <- mod_readout$sample(data = data_bio, chains = 4, parallel_chains = 4, iter_warmup = 1000, iter_sampling = 1000, refresh=500)
loo_bio <- fit_bio$loo()
cat("Bio ELPD:\n")
print(loo_bio)

cat("Running Stan QLearn Model (Hierarchical + Intercept, In-Dist N=10)...\n")
data_qlearn <- list(N = N_final, K = 10, D = 8, subj_id = subj_mapped, y = y_switch_final, trace = q_hist)
...
cat("Running Stan WSLS Model (Hierarchical, In-Dist N=10)...\n")
data_wsls <- list(N = N_final, K = 10, subj_id = subj_mapped, y = y_choice_final,
...
cat("\n===========================\nLOO COMPARISON (In-Dist N=10, Hierarchical + Intercept)\n===========================\n")
print(comp)

saveRDS(list(Bio = loo_bio, WSLS = loo_wsls, QLearn = loo_qlearn, comp = comp), "final_elpd_indist_hierarchical_n10_intercept.rds")
