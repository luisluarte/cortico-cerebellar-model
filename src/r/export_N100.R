...
all_logits <- numeric(); all_mu <- numeric(); all_sigma <- numeric()
all_labels <- numeric(); all_lag_reward <- numeric(); all_lag_ch <- numeric()
all_lag_resp <- numeric(); all_subjs <- numeric(); all_X <- matrix(nrow = 0, ncol = ncol(X))
all_ITI <- numeric()

cat("Computing precise distribution targets for all N=100 subjects...\n")
unique_subs <- unique(stan_data$subj)
...
saveRDS(targets, "distillation_targets_N100.rds")
cat("Saved to distillation_targets_N100.rds\n")
