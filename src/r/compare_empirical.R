...
cat("Setting up data for all models...\n")
...
run_mcmc <- function(eval_fn, N_params, d, iters=100) {
  ...
  # Use mu_EB baseline to speed it up. Or zero. Wait, let's use zeros for QLearn and WSLS
  mu_pop <- rep(0, N_params)
  sigma_pop <- rep(-3, N_params) # Tight prior
...
cat("Running Empirical Q-Learning MCMC...\n")
q_trace <- run_mcmc(eval_q, 3, data_list, 100)
cat("Running Empirical WSLS MCMC...\n")
wsls_trace <- run_mcmc(eval_wsls, 2, data_list, 100)

cat("Re-Running Empirical Biological C++ MCMC...\n")
set.seed(42)
...
sourceCpp("empirical_mcmc.cpp")
bio_res <- run_empirical_mcmc(100, length(test_subjs), data_list$subj_indices, data_list$X, data_list$ITI, 
...
cat("\n--- IN-BAG EMPIRICAL MODEL COMPARISON ---\n")
