...
// [[Rcpp::export]]
List run_empirical_mcmc_unconstrained(int iters, int N_subjs, uvec subj_indices, 
  ...
  vec sigma_pop = zeros<vec>(7); sigma_pop.fill(-1.0); // Wider unconstrained variance
  mat Z_sub = randn<mat>(N_subjs, 7);
  ...
  for(int iter = 0; iter < iters; iter++) {
    ...
    double prior_mu_old = accu(-0.5 * square(mu_pop) / 9.0);
    double prior_mu_new = accu(-0.5 * square(mu_pop_new) / 9.0);
