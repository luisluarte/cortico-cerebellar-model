...
// [[Rcpp::export]]
List run_bio_mcmc_chain(int iters, NumericMatrix X_r, NumericVector ITI_r, NumericVector reward_r,
  ...
  vec sigma_pop = zeros<vec>(7); sigma_pop.fill(-2.0); // Allow reasonable exploration range in logit
  ...
  // Starting point
  vec Z_curr = randn<vec>(7); // start random for proper Rhat convergence checks
  ...
  for(int iter = 0; iter < iters; iter++) {
    vec Z_new = Z_curr + randn<vec>(7) * step_size;
    ...
    // In Chaotic mode, we also have to move mu_pop if it's flat, but since this is per-subject, 
    // it's functionally identical to moving Z when mu_pop=0. We'll just let Z explore.
    
    if(log(randu()) < (ll_new + prior_new) - (ll_curr + prior_old)) {
        Z_curr = Z_new;
        ll_curr = ll_new;
    }
    
    trace_z.row(iter) = Z_curr.t();
    trace_ll.row(iter).fill(ll_curr / N_trials); // Approximation of pointwise for LOO
  }
