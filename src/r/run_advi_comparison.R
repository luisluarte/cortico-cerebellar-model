...
cat("Fitting Bio Model via ADVI...\n")
data_bio <- list(N = N_final, K = 5, D = 32, subj_id = subj_mapped, y = y_switch_final, trace = mu_hist)
fit_bio <- mod_readout$variational(data = data_bio, seed = 123, iter = 20000, output_samples = 1000)

cat("Fitting QLearn Model via ADVI...\n")
data_qlearn <- list(N = N_final, K = 5, D = 8, subj_id = subj_mapped, y = y_switch_final, trace = q_hist)
fit_qlearn <- mod_readout$variational(data = data_qlearn, seed = 123, iter = 20000, output_samples = 1000)

cat("Fitting WSLS Model via ADVI...\n")
...
data_wsls <- list(
  ...
  q3_theta_win_logit = logit(q3_wsls$theta_win),
  q3_theta_loss_logit = logit(q3_wsls$theta_loss)
)
fit_wsls <- mod_wsls$variational(data = data_wsls, seed = 123, iter = 20000, output_samples = 1000)

cat("\n=========================================\n")
cat("COMPUTING LOO / ELPD\n")
cat("=========================================\n")

# extract log_lik 
ll_bio <- fit_bio$draws("log_lik")
ll_qlearn <- fit_qlearn$draws("log_lik")
ll_wsls <- fit_wsls$draws("log_lik")

# If format is draws_matrix, loo function works out of the box
loo_bio <- loo(ll_bio)
loo_qlearn <- loo(ll_qlearn)
loo_wsls <- loo(ll_wsls)
...
cat("=========================================\n")
comp <- loo_compare(list(Bio = loo_bio, QLearn = loo_qlearn, WSLS = loo_wsls))
print(comp)
