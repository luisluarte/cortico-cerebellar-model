...
cat("Setting up data for unconstrained empirical models...\n")
...
cat("Running Empirical Q-Learning MCMC (Unconstrained)...\n")
q_trace <- run_mcmc_unconstrained(eval_q, 3, data_list, 500)
cat("Running Empirical WSLS MCMC (Unconstrained)...\n")
wsls_trace <- run_mcmc_unconstrained(eval_wsls, 2, data_list, 500)

cat("Re-Running Empirical Biological C++ MCMC (Unconstrained)...\n")
set.seed(42)
...
sourceCpp("empirical_mcmc_unconstrained.cpp")
bio_res <- run_empirical_mcmc_unconstrained(500, length(test_subjs), data_list$subj_indices, data_list$X, data_list$ITI, 
                                 data_list$y_choice, data_list$y_rt, Pi_mat,
                                 W_gen, W_ach1, W_ach2, W_thal, data_list$N_total)

# Burn-in: discard first 250 iterations for LOO calculation
burn <- 251:500
...
cat("\n--- IN-BAG EMPIRICAL UNCONSTRAINED MODEL COMPARISON ---\n")
