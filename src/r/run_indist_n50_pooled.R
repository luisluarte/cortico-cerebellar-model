...
cat("Loading Stan POOLED Models...\n")
mod_readout <- cmdstan_model("readout_pooled.stan")
mod_wsls <- cmdstan_model("wsls_pooled.stan")

cat("Running Stan Bio Model (POOLED, In-Dist N=50)...\n")
data_bio <- list(N = N_final, D = 32, y = y_switch_final, trace = mu_hist)
fit_bio <- mod_readout$sample(data = data_bio, chains = 4, parallel_chains = 4, iter_warmup = 1000, iter_sampling = 1000, refresh=500)
loo_bio <- fit_bio$loo()
cat("Bio ELPD:\n")
print(loo_bio)

cat("Running Stan QLearn Model (POOLED, In-Dist N=50)...\n")
data_qlearn <- list(N = N_final, D = 8, y = y_switch_final, trace = q_hist)
...
cat("Running Stan WSLS Model (POOLED, In-Dist N=50)...\n")
data_wsls <- list(N = N_final, y = y_switch_final,
...
cat("\n===========================\nLOO COMPARISON (In-Dist N=50, POOLED)\n===========================\n")
print(comp)

saveRDS(list(Bio = loo_bio, WSLS = loo_wsls, QLearn = loo_qlearn, comp = comp), "final_elpd_indist_pooled_n50.rds")
