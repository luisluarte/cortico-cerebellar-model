...
cat("Loading N=100 Distilled Data & isolating 3 In-Distribution subjects...\n")
...
set.seed(42)
selected_3_indist <- sample(train_subjs, 3)
valid_idx <- which(lag_Resp >= 0 & stan_data$subj %in% selected_3_indist & lag_Ch > 0)
...
cat(sprintf("Retained %d trials across %d participants.\n", N_final, K_final))
...
cat("Generating fixed network weights...\n")
set.seed(42)
...
init_fun <- function() {
  list(
    ...
    sigma_alpha_pc = 0.01,
    sigma_lambda_pc = 0.01,
    sigma_beta_thal = 0.01,
    sigma_kappa_cf = 0.01,
    sigma_alpha_gran = 0.01,
    sigma_beta_gran = 0.01,
    
    alpha_pc_raw = rep(0, K_final),
    lambda_pc_raw = rep(0, K_final),
    beta_thal_raw = rep(0, K_final),
    kappa_cf_raw = rep(0, K_final),
    alpha_gran_raw = rep(0, K_final),
    beta_gran_raw = rep(0, K_final),
    
    alpha_pop = 0,
    sigma_alpha = 0.1,
    alpha_raw = rep(0, K_final),
    W_readout_pop = rep(0, 32),
    sigma_W_readout = rep(0.1, 32),
    W_readout_raw = matrix(0, nrow=32, ncol=K_final)
  )
}

cat("Compiling Full Hierarchical Bio Stan Model...\n")
mod_full <- cmdstan_model("full_bio_hierarchical.stan")

cat("Running Stan baseline timing test (N=3)...\n")
...
cat("TIMING RESULTS\n")
cat("=========================================\n")
time_info <- fit_full$time()
cat(sprintf("Total Warmup Time (100 iter):   %.2f sec\n", time_info$chains$warmup))
cat(sprintf("Total Sampling Time (100 iter): %.2f sec\n", time_info$chains$sampling))
cat(sprintf("Average Time per Iteration:     %.3f sec\n", (time_info$chains$warmup + time_info$chains$sampling) / 200))
