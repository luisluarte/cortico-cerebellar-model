...
pA <- ggplot(sum_abs, aes(x = Delta_Beta, y = emmean, color = Condition, fill = Condition)) +
  ...
  labs(
    title = "A. Non-Linear Behavioral Curves (Simplified Architecture)",
    x = "",
...
p_final <- pA / pB + plot_annotation(
  ...
  subtitle = "Final Simplified Architecture. Divergences: 0. Rhat: 1.00.",
  theme = theme(
...
ggsave("Fig36_Spline_Simplified_Final.png", p_final, width = 10, height = 9, dpi = 300)
