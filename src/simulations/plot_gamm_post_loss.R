...
cat("Preparing True P(Switch | Loss) Data (Top 50% Native Beta)...\n")
...
# FILTER FOR POST-LOSS ONLY
df_loss <- df_all %>% filter(!is.na(Prev_F) & Prev_F == 0)

med_db <- median(unique(df_all$Delta_Beta))
# Filter for Option 1: Top 50% native beta
df_window <- df_loss %>% filter(Time_Since_Rev >= 0 & Time_Since_Rev <= 15 & Delta_Beta > med_db)
...
# PLOT 76A: The actual P(Switch | Loss) curves
cat("Extracting Absolute Curves...\n")
preds_opt <- predict(fit_loss, newdata = nd_opt, type = "response", se.fit=TRUE, exclude="s(Subject)")
preds_les <- predict(fit_loss, newdata = nd_les, type = "response", se.fit=TRUE, exclude="s(Subject)")
...
out_path_a <- "C:/Users/DCCS5/.gemini/antigravity/brain/ee7b6b70-a0ae-4607-9cdf-f55667b1cc2c/Fig76A_GAMM_PostLoss_Curves.png"
p_curves <- ggplot(df_curves, aes(x = Time_Since_Rev, y = Fit, color = Condition, fill=Condition)) +
  ...
  labs(
    ...
    subtitle = "Intact Network vs Compensated Lesion (High Native Buffer N=25)",
...
# PLOT 76B: The Difference Curve
cat("Extracting Difference Curve...\n")
Xp_opt <- predict(fit_loss, newdata = nd_opt, type = "lpmatrix", exclude = "s(Subject)")
Xp_les <- predict(fit_loss, newdata = nd_les, type = "lpmatrix", exclude = "s(Subject)")

Xp_diff <- Xp_opt - Xp_les
diff_est <- Xp_diff %*% coef(fit_loss)
diff_se <- sqrt(rowSums((Xp_diff %*% vcov(fit_loss)) * Xp_diff))
...
out_path_b <- "C:/Users/DCCS5/.gemini/antigravity/brain/ee7b6b70-a0ae-4607-9cdf-f55667b1cc2c/Fig76B_GAMM_PostLoss_Difference.png"
p_diff <- ggplot(df_plot, aes(x = Time_Since_Rev, y = Diff)) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "black", linewidth = 1) +
  geom_ribbon(aes(ymin = CI_lower, ymax = CI_upper), fill = "#7570B3", alpha = 0.3) +
  geom_line(color = "#7570B3", linewidth = 1.5) +
  theme_minimal(base_size = 14) +
  labs(
    title = "Contrast Curve: P(Switch | Loss)",
    subtitle = "\u0394 Log-Odds (Intact - Lesioned)",
    x = "Trials Since Environmental Reversal",
