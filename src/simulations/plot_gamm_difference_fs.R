...
# NO MORE MEDIAN FILTER! Include ALL 50 SUBJECTS!
...
cat("Fitting GAMM (with Factor Smooth for ALL 50 Subjects)...\n")
fit_gamm_fs <- gam(
  P_Switch ~ Condition + s(Time_Since_Rev, by = Condition, k = 5) + 
             s(Time_Since_Rev, Subject, bs = "fs", m = 1),
  data = df_long,
...
Xp_opt <- predict(fit_gamm_fs, newdata = nd_opt, type = "lpmatrix", exclude = "s(Time_Since_Rev,Subject)")
Xp_les <- predict(fit_gamm_fs, newdata = nd_les, type = "lpmatrix", exclude = "s(Time_Since_Rev,Subject)")

Xp_diff <- Xp_opt - Xp_les

diff_est <- Xp_diff %*% coef(fit_gamm_fs)
diff_se <- sqrt(rowSums((Xp_diff %*% vcov(fit_gamm_fs)) * Xp_diff))
...
out_path <- "C:/Users/DCCS5/.gemini/antigravity/brain/ee7b6b70-a0ae-4607-9cdf-f55667b1cc2c/Fig73_GAMM_FS_50Subjs_Difference_Curve.png"

p <- ggplot(df_plot, aes(x = Time_Since_Rev, y = Diff)) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "black", linewidth = 1) +
  geom_ribbon(aes(ymin = CI_lower, ymax = CI_upper), fill = "#1B9E77", alpha = 0.3) +
  geom_line(color = "#1B9E77", linewidth = 1.5) +
  theme_minimal(base_size = 14) +
  labs(
    title = "Contrast Curve: Intact vs. Compensated Lesion (N=50)",
    subtitle = "Difference in Log-Odds with Subject-Level Random Smooths",
    x = "Trials Since Environmental Reversal",
    y = "\u0394 Log-Odds of Switching (Intact - Lesion)"
  ) +
