...
cat("1. Building Robust Simulation Data (Clamped)...\n")
...
for(s_idx in 1:length(unique_subjs)) {
  ...
  # Widen the reversal window to capture more dynamics!
  rev_window <- unique(unlist(lapply(rev_idx, function(x) seq(x, min(x+10, nrow(d_emp))))))
  ...
  if(length(t_all) > 10) {
      ...
      df_opt <- data.frame(Subject=s_name, Trial=t_all, State=state, Condition="Optimized", 
                           P_Switch=p_sw_opt, Delta_Beta=delta_beta, F=feedback_prev)
      df_les <- data.frame(Subject=s_name, Trial=t_all, State=state, Condition="Lesioned", 
                           P_Switch=p_sw_les, Delta_Beta=delta_beta, F=feedback_prev)
...
long_df$P_Switch_Sq <- (long_df$P_Switch * (nrow(long_df) - 1) + 0.5) / nrow(long_df)

saveRDS(long_df, "results/reversal_df.rds")
cat("Saved reversal_df.rds (N=", nrow(long_df), ")\n", sep="")
