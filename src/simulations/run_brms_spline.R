...
cat("Preparing Data for Bayesian GAM...\n")
...
cat("Starting brms Bayesian GAM fit... \n")
# Bayesian Spline (GAM equivalent)
fit_spline <- brm(
  formula = P_Switch ~ Condition + s(Time_Since_Rev, by = Condition, k = 5) + (1 | Subject),
  data = df_long,
...
saveRDS(fit_spline, "results/brms_reversal_spline.rds")

cat("\n================ BAYESIAN GAM SUMMARY ================\n")
