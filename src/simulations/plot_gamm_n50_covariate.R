...
cat("Fitting GAMM (Post-Loss N=50 with Delta_Beta Covariate)...\n")
fit_cov <- gam(
  P_Switch ~ Condition + 
...
out_path_a <- "C:/Users/DCCS5/.gemini/antigravity/brain/ee7b6b70-a0ae-4607-9cdf-f55667b1cc2c/Fig77A_GAMM_N50_Curves.png"
p_curves <- ggplot(df_curves, aes(x = Time_Since_Rev, y = Fit, color = Condition, fill=Condition)) +
  ...
  labs(
    title = "P(Switch | Loss) Reversal Time-Course",
    subtitle = "Covariate-Controlled (N=50), Evaluated at 75th Percentile \u0394\u03B2",
    x = "Trials Since Environmental Reversal",
    y = "P(Switch | Loss)"
  )
ggsave(out_path_a, p_curves, width=8, height=5, bg="white")

target_db_high <- max(df_long$Delta_Beta)
target_db_low <- min(df_long$Delta_Beta)
...
df_high <- data.frame(
  ...
  Lesion_Size = "Maximum Lesion (Max Empirical \u0394\u03B2)"
)
...
Xp_opt_l <- predict(fit_cov, newdata = nd_opt_low, type = "lpmatrix", exclude = "s(Subject)")
Xp_les_l <- predict(fit_cov, newdata = nd_les_low, type = "lpmatrix", exclude = "s(Subject)")

Xp_diff_l <- Xp_opt_l - Xp_les_l
diff_est_l <- Xp_diff_l %*% coef(fit_cov)
diff_se_l <- sqrt(rowSums((Xp_diff_l %*% vcov(fit_cov)) * Xp_diff_l))

df_low <- data.frame(
  Time_Since_Rev = nd_opt_low$Time_Since_Rev,
  Diff = as.numeric(diff_est_l),
  CI_lower = as.numeric(diff_est_l - 1.96 * diff_se_l),
  CI_upper = as.numeric(diff_est_l + 1.96 * diff_se_l),
  Lesion_Size = "No Lesion (Min Empirical \u0394\u03B2)"
)

df_plot_both <- bind_rows(df_high, df_low)
df_plot_both$Lesion_Size <- factor(df_plot_both$Lesion_Size, levels = c("Maximum Lesion (Max Empirical \u0394\u03B2)", "No Lesion (Min Empirical \u0394\u03B2)"))

out_path_b <- "C:/Users/DCCS5/.gemini/antigravity/brain/ee7b6b70-a0ae-4607-9cdf-f55667b1cc2c/Fig79_GAMM_MinMax_Difference.png"
p_diff <- ggplot(df_plot_both, aes(x = Time_Since_Rev, y = Diff, color = Lesion_Size, fill = Lesion_Size)) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "black", linewidth = 1) +
  geom_ribbon(aes(ymin = CI_lower, ymax = CI_upper), alpha = 0.2, color = NA) +
  geom_line(linewidth = 1.5) +
  scale_color_manual(values = c("Maximum Lesion (Max Empirical \u0394\u03B2)" = "#7570B3", "No Lesion (Min Empirical \u0394\u03B2)" = "#D95F02")) +
  scale_fill_manual(values = c("Maximum Lesion (Max Empirical \u0394\u03B2)" = "#7570B3", "No Lesion (Min Empirical \u0394\u03B2)" = "#D95F02")) +
  theme_minimal(base_size = 14) +
  labs(
    title = "Extreme Bounds Contrast: P(Switch | Loss)",
    subtitle = "Difference in \u0394 Log-Odds evaluated at Empirical Min vs Max",
    x = "Trials Since Environmental Reversal",
    y = "\u0394 Log-Odds (Intact - Lesion)",
    color = "Effective Lesion Size",
    fill = "Effective Lesion Size"
  ) +
  theme(legend.position = "bottom")

ggsave(out_path_b, p_diff, width=9, height=6, bg="white")
cat("Saved Dose-Response plot.\n")
