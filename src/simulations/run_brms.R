...
cat("Fitting Distributional Bayesian Hierarchical Beta Regression (Logistic link)...\n")
# Using the powerful remote Arch server!
# 4 chains, 4 cores, 2000 iter to get a highly robust posterior
# We add control = list(adapt_delta = 0.95) because complex random slopes can cause divergences

brm_form <- bf(P_Switch ~ Condition * State * log_Delta_Beta + (Condition * State | Subject)) + 
            lf(phi ~ 1 + (1 | Subject))
...
# Extract summary and save model to disk for pulling
saveRDS(fit_brm, "results/brms_switch_model.rds")
sink("results/brms_switch_summary.txt")
