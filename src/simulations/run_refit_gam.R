...
# Filter to window and vulnerable half
med_db <- median(unique(df_all$Delta_Beta))
df_window <- df_all %>% filter(Time_Since_Rev >= 0 & Time_Since_Rev <= 20 & Delta_Beta <= med_db)
...
cat("Fitting GAM...\n")
m_gam <- gam(P_Switch ~ Condition + s(Time_Since_Rev, by = Condition, k = 5), 
             data = df_long, family = betar(link="logit"))

nd <- expand.grid(Time_Since_Rev = seq(0, 20, by = 0.5), Condition = factor(c("Optimized", "Lesioned_Refit")))
...
p <- ggplot(nd, aes(x = Time_Since_Rev, y = fit, color = Condition, fill = Condition)) +
  ...
  labs(
    title = "Reversal Time-Course: Compensated Lesion vs Intact",
    subtitle = "Even with an adapted readout, the lesion destroys deep temporal integration (priors)",
    x = "Trials Since Environmental Reversal",
    y = "P(Switch | Loss)"
  ) +
  theme(legend.position = "bottom")

ggsave("Fig69_Refit_Reversal_GAM.png", p, width = 8, height = 6, dpi = 300)
cat("Done.\n")
