...
# The response variable is P_Calibrated
...
p <- ggplot(df_emm, aes(x = Condition, y = emmean, color = Condition)) +
  ...
  labs(title = "Bayesian Posterior Calibrated P(Switch) by Ablation Magnitude",
       y = "Calibrated P(Switch) Mean & 95% HPD") +
  scale_color_manual(values=c("Lesioned"="#d95f02", "Optimized"="#1b9e77")) +
  theme(strip.text = element_text(face="bold"))

ggsave("Fig23_Calibrated_Bayesian_Emmeans.png", p, width=11, height=7, dpi=300)
