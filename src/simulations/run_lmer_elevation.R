...
# Shift focus to the actual area under the curve
# T=0 is pre-adaptation, T=1 to 5 is where the contrast happens
df_trunc <- df_loss %>% filter(Time_Since_Rev >= 1 & Time_Since_Rev <= 5)

df_long <- bind_rows(
  df_trunc %>% select(Subject, Delta_Beta, P_Switch = P_Opt) %>% mutate(Condition = "Optimized"),
  df_trunc %>% select(Subject, Delta_Beta, P_Switch = P_Les) %>% mutate(Condition = "Lesioned_Refit")
)
...
cat("\n============================================\n")
cat("Fitting LMER (T=1 to T=5 Average Shift)\n")
cat("Formula: P_Switch ~ Condition * Delta_Beta + (1 | Subject)\n")
cat("============================================\n")

fit_lmer <- lmer(P_Switch ~ Condition * Delta_Beta + (1 | Subject), data = df_long)
