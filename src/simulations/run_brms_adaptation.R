...
for(s_idx in 1:length(unique_subjs)) {
  ...
  for(i in 1:length(idx_s)) {
    ...
    if(length(past_revs) > 0) {
      time_since_rev[i] <- t - max(past_revs)
    }
  }
...
# Filter to vulnerability and early reversal window (0 to 10 trials)
med_db <- median(unique(df_all$Delta_Beta))
df_window <- df_all %>% filter(Time_Since_Rev >= 0 & Time_Since_Rev <= 10 & Delta_Beta <= med_db)
...
# Scale to (0,1) for Beta regression
N <- nrow(df_long)
df_long$P_Switch <- (df_long$P_Switch * (N - 1) + 0.5) / N

cat("Starting brms fit (Adaptation Slope)... \n")
# Using 2 chains and 2 cores to adhere to RAM limits
fit_reversal <- brm(
  formula = P_Switch ~ Time_Since_Rev * Condition + (Time_Since_Rev * Condition | Subject),
  data = df_long,
...
saveRDS(fit_reversal, "results/brms_reversal_adaptation.rds")

cat("\n================ MODEL SUMMARY ================\n")
print(summary(fit_reversal))

cat("\n================ HYPOTHESIS TEST ================\n")
cat("Testing if the Lesioned adaptation slope is significantly flatter (more positive/closer to zero) than the Intact slope.\n")
hyp <- hypothesis(fit_reversal, "Time_Since_Rev:ConditionLesioned_Refit > 0")
print(hyp)
