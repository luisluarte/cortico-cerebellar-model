...
for(s_idx in 1:length(unique(targets$subjs))) {
  ...
  for(i in 1:length(f_s)) {
    if(f_s[i] == 0) current_streak <- current_streak + 1 else current_streak <- 0
  ...
  df_subj <- data.frame(
    Subject = as.character(s_num), Loss_Streak = loss_streak,
    P_Opt = p_switch_opt, P_Les_Comp = p_switch_les
  )
  all_trials[[length(all_trials) + 1]] <- df_subj
}

df_all <- bind_rows(all_trials) %>% filter(!is.na(P_Opt) & Loss_Streak > 0)
df_all$Streak_Bin <- ifelse(df_all$Loss_Streak >= 5, "5+", as.character(df_all$Loss_Streak))

cat("\n--- P(Switch) by Loss Streak Length ---\n")
print(df_all %>% group_by(Streak_Bin) %>% summarize(
  N = n(),
  Intact = mean(P_Opt, na.rm=TRUE),
  Lesioned = mean(P_Les_Comp, na.rm=TRUE)
))
