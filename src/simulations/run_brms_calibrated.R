...
# CALIBRATION: Normalize the dynamic range of each participant
long_df <- long_df %>%
  group_by(Subject) %>%
  mutate(
    P_min = min(P_Switch),
    P_max = max(P_Switch),
    # Scale each participant's probabilities so they all operate between 0 and 1
    P_Calibrated = (P_Switch - P_min) / (P_max - P_min + 1e-8),
    # Squeeze into the open (0,1) interval for Beta regression
    P_Calibrated = pmax(pmin(P_Calibrated, 0.9999), 0.0001)
  ) %>% ungroup()
...
cat("Fitting Calibrated Bayesian Hierarchical Beta Regression...\n")
# Using the pre-processed, scale-calibrated probability!
# This allows us to revert back to the simple (1 | Subject) model because the dynamic range is now fixed empirically!
fit_brm <- brm(
  P_Calibrated ~ Condition * State * log_Delta_Beta + (1 | Subject),
  data = long_df,
...
saveRDS(fit_brm, "results/brms_switch_model_calibrated.rds")
sink("results/brms_switch_summary_calibrated.txt")
