...
# Scale density to sit in the bottom 20% of the y-axis
...
p <- ggplot(sum_diff, aes(x = Delta_Beta)) +
  ...
  labs(
    title = "The Cerebellar Shock Absorber",
    subtitle = bquote(Delta ~ "P(Switch) [Optimized - Lesioned]. Grey area shows empirical subject density."),
    x = bquote("Structural Integrity (" ~ Delta*beta[thal] ~ ")  [Health -> Damage]"),
    y = bquote("Change in P(Switch) [Cerebellar Correction]")
  )

ggsave("Fig62_Cerebellar_Correction_Density.png", p, width = 9, height = 6, dpi = 300)
cat("Done.\n")
