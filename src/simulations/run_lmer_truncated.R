...
# TRUNCATE AT T=5
df_trunc <- df_loss %>% filter(Time_Since_Rev >= 0 & Time_Since_Rev <= 5)

df_long <- bind_rows(
  df_trunc %>% select(Subject, Time_Since_Rev, Delta_Beta, P_Switch = P_Opt) %>% mutate(Condition = "Optimized"),
  df_trunc %>% select(Subject, Time_Since_Rev, Delta_Beta, P_Switch = P_Les) %>% mutate(Condition = "Lesioned_Refit")
)

# Set Lesioned as baseline so 'ConditionOptimized' represents the positive effect
df_long$Condition <- factor(df_long$Condition, levels=c("Lesioned_Refit", "Optimized"))
df_long$Subject <- as.factor(df_long$Subject)

cat("\n============================================\n")
cat("Fitting LMER (T=0 to T=5)\n")
cat("Formula: P_Switch ~ Condition * Time_Since_Rev * Delta_Beta + (1 | Subject)\n")
cat("============================================\n")

fit_lmer <- lmer(P_Switch ~ Condition * Time_Since_Rev * Delta_Beta + (1 | Subject), data = df_long)
summary_fit <- summary(fit_lmer)
print(summary_fit)

capture.output(summary_fit, file="results/lmer_summary_truncated.txt")
cat("\nSaved summary to results/lmer_summary_truncated.txt\n")
