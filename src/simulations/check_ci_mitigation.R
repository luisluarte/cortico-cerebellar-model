...
# TEST 1: High Beta (Healthy natives) -> Lesion contrast
df_high <- df_all %>% filter(Time_Since_Rev >= 0 & Time_Since_Rev <= 15 & Delta_Beta > med_db)
df_high_long <- bind_rows(
  df_high %>% select(Subject, Time_Since_Rev, P_Switch = P_Opt) %>% mutate(Condition = "Optimized"),
  df_high %>% select(Subject, Time_Since_Rev, P_Switch = P_Les) %>% mutate(Condition = "Lesioned_Refit")
)
df_high_long$Condition <- factor(df_high_long$Condition, levels=c("Optimized", "Lesioned_Refit"))
df_high_long$Subject <- as.factor(df_high_long$Subject)
N <- nrow(df_high_long)
df_high_long$P_Switch <- (df_high_long$P_Switch * (N - 1) + 0.5) / N

fit_high <- gam(P_Switch ~ Condition + s(Time_Since_Rev, by = Condition, k = 5) + s(Subject, bs = "re"), data = df_high_long, family = betar(link = "logit"), method = "REML")

nd_opt <- data.frame(Time_Since_Rev = seq(0, 15, length.out = 100), Condition = factor("Optimized", levels = levels(df_high_long$Condition)), Subject = df_high_long$Subject[1])
nd_les <- nd_opt
nd_les$Condition <- factor("Lesioned_Refit", levels = levels(df_high_long$Condition))

Xp_opt <- predict(fit_high, newdata = nd_opt, type = "lpmatrix", exclude = "s(Subject)")
Xp_les <- predict(fit_high, newdata = nd_les, type = "lpmatrix", exclude = "s(Subject)")
Xp_diff <- Xp_opt - Xp_les
diff_est <- Xp_diff %*% coef(fit_high)
diff_se <- sqrt(rowSums((Xp_diff %*% vcov(fit_high)) * Xp_diff))
df_plot <- data.frame(Time_Since_Rev = nd_opt$Time_Since_Rev, Diff = as.numeric(diff_est), CI_lower = as.numeric(diff_est - 1.96 * diff_se), CI_upper = as.numeric(diff_est + 1.96 * diff_se))

cat("\n--- High Native Beta (Healthy) Contrast ---\n")
print(head(df_plot %>% filter(Time_Since_Rev > 3 & Time_Since_Rev < 8)))
