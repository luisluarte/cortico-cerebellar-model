...
cat("Fitting GAMM (with Random Effect for Subject)...\n")
fit_gamm <- gam(
  P_Switch ~ Condition + s(Time_Since_Rev, by = Condition, k = 5) + s(Subject, bs = "re"),
  data = df_long,
  family = betar(link = "logit"),
  method = "REML"
)

cat("Calculating Difference Curve (Intact - Lesion)...\n")
# Create dummy data for prediction (ignoring subject random effect)
nd_opt <- data.frame(
  Time_Since_Rev = seq(0, 15, length.out = 100),
  Condition = factor("Optimized", levels = levels(df_long$Condition)),
  Subject = df_long$Subject[1] # Dummy, excluded in predict
)
nd_les <- nd_opt
nd_les$Condition <- factor("Lesioned_Refit", levels = levels(df_long$Condition))

# Extract the linear predictor matrix (lpmatrix)
Xp_opt <- predict(fit_gamm, newdata = nd_opt, type = "lpmatrix", exclude = "s(Subject)")
Xp_les <- predict(fit_gamm, newdata = nd_les, type = "lpmatrix", exclude = "s(Subject)")

# The difference matrix isolates the pure difference in shape and intercept
Xp_diff <- Xp_opt - Xp_les

# Calculate the difference and its standard error on the logit scale
diff_est <- Xp_diff %*% coef(fit_gamm)
diff_se <- sqrt(rowSums((Xp_diff %*% vcov(fit_gamm)) * Xp_diff))
...
out_path <- "C:/Users/DCCS5/.gemini/antigravity/brain/ee7b6b70-a0ae-4607-9cdf-f55667b1cc2c/Fig71_GAMM_Difference_Curve.png"

p <- ggplot(df_plot, aes(x = Time_Since_Rev, y = Diff)) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "black", size = 1) +
  geom_ribbon(aes(ymin = CI_lower, ymax = CI_upper), fill = "#2E86AB", alpha = 0.3) +
  geom_line(color = "#2E86AB", size = 1.5) +
  theme_minimal(base_size = 14) +
  labs(
    title = "Contrast Curve: Intact vs. Compensated Lesion",
    subtitle = "Difference in Log-Odds (Intact - Lesion) via Mixed-Effects GAM",
    x = "Trials Since Environmental Reversal",
    y = "\u0394 Log-Odds of Switching"
  ) +
