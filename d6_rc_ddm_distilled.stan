  ...
  real subject_lpmf(array[] int subj_slice, int start, int end,
    ...
    for (s_idx in 1:size(subj_slice)) {
      ...
      for (t in 1:n_t) {
        ...
        // Distilled Policy
        lp += soft_p_all[idx] * log_inv_logit(v_t * a_dyn) + (1 - soft_p_all[idx]) * log1m_inv_logit(v_t * a_dyn);
        
        // Empirical Kinematics (Conditional RT Likelihood)
        if (RT_all[idx] > ndt) {
          real log_prob_choice;
          real log_wiener;
          if (Resp_all[idx] == 1) {
            log_prob_choice = log_inv_logit(v_t * a_dyn);
            log_wiener = wiener_lpdf(RT_all[idx] | a_dyn, ndt, 0.5, v_t);
          } else {
            log_prob_choice = log1m_inv_logit(v_t * a_dyn);
            log_wiener = wiener_lpdf(RT_all[idx] | a_dyn, ndt, 0.5, -v_t);
          }
          lp += (log_wiener - log_prob_choice);
        } else {
