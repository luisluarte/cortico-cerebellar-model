...
parameters {
  ...
  real<lower=0, upper=1> lambda; // Destabilization factor
...
model {
  ...
  for (t in 1:N) {
    ...
    // M4: Repulsive Search Vector Field
    if (E_t > 0) {
      theta = theta + alpha_pos * E_t * W_exp[:, ch];
    } else {
      theta = lambda * theta + alpha_neg * E_t * W_exp[:, ch];
    }
  }
}
generated quantities {
  ...
  for (t in 1:N) {
    ...
    if (E_t > 0) {
      ...
      theta_gen = lambda * theta_gen + alpha_neg * E_t * W_exp[:, ch];
    }
  }
}
