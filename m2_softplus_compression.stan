...
model {
  ...
  for (t in 1:N) {
    ...
    // M2: Smooth Excitatory-Only Softplus Compression
    vector[n_dim] active_theta = log1p_exp(theta);
    
    real V_ch = dot_product(active_theta, W_exp[:, ch]);
    real V_unch = dot_product(active_theta, W_exp[:, unch]);
...
generated quantities {
  ...
  for (t in 1:N) {
    ...
    vector[n_dim] active_theta_gen = log1p_exp(theta_gen);
    
    real V1 = dot_product(active_theta_gen, W_exp[:, c1]);
    real V2 = dot_product(active_theta_gen, W_exp[:, c2]);
    ...
    real V_ch = dot_product(active_theta_gen, W_exp[:, ch]);
    real E_t = Reward[t] - V_ch;
    if (E_t > 0) {
