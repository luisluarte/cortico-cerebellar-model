...
model {
  ...
  for (t in 1:N) {
    ...
    // 1. Empirical Human Data Log-Likelihood (RT & Choice via Wiener)
    if (Resp[t] == 1) {
      RT[t] ~ wiener(a, ndt, 0.5, v_t_full);
    } else {
      RT[t] ~ wiener(a, ndt, 0.5, -v_t_full);
    }
    
    // 2. Continuous Distillation Objective (Minimize KL Divergence)
    // DDM Probability of Choice 1 is inv_logit(v_t_full * a)
