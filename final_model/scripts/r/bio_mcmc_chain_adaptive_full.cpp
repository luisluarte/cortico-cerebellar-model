...
List run_bio_mcmc_chain(int iters, NumericMatrix X_r, NumericVector ITI_r, NumericVector reward_r,
  ...
  if(prior_mode == "flat") {
      ...
      sigma_pop.fill(-3.0); // Increased variance from -5.0
  } else if(prior_mode == "anchored") {
      sigma_pop.fill(0.0);  // Increased variance from -1.0
  }
  ...
  // Tight initialization near the prior mode
  ...
  for(int iter = 0; iter < iters; iter++) {
    ...
    // Adaptive step size targeting 23.4% during burn-in
    if(iter < burn_in) {
