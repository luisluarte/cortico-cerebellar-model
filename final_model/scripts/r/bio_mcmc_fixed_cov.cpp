...
mat simulate_bio_subject_fixed(int N_trials, mat X, vec ITI, vec reward,
    ...
    for(int t = 0; t < N_trials; t++) {
        if(ITI(t) > 0) {
            Z.zeros();
            W_purk += noise_mat.col(t) * sqrt(p_sigma2 * ITI(t));
        }
...
// [[Rcpp::export]]
List run_bio_mcmc_fixed_cov(int iters, NumericMatrix X_r, NumericVector ITI_r, NumericVector reward_r,
  ...
  // PRE-GENERATE FIXED NOISE MATRIX SO LIKELIHOOD IS DETERMINISTIC!
  mat noise_mat = randn<mat>(896, N_trials);
  
  vec mu_pop = zeros<vec>(7);
  if(prior_mode == "locked" || prior_mode == "anchored") mu_pop = mu_EB;
  
  vec sigma_pop = zeros<vec>(7);
  if(prior_mode == "locked") sigma_pop.fill(-3.0); 
  ...
  mat bio_mu_curr = simulate_bio_subject_fixed(N_trials, X, ITI, reward, W_gen, W_ach1, W_ach2, W_thal, Pi_mat, noise_mat, params_curr);
  double ll_curr = eval_distillation(bio_mu_curr, rnn_mu);
  ...
  for(int iter = 0; iter < iters; iter++) {
    ...
    if (iter < 500) {
        ...
        if (randu() < 0.95) {
            ...
            step = randn<vec>(7) * 0.01; 
        }
    }
    
    vec Z_new = Z_curr + step;
    vec params_new = inv_logit_scaled(mu_pop + exp(sigma_pop) % Z_new);
    mat bio_mu_new = simulate_bio_subject_fixed(N_trials, X, ITI, reward, W_gen, W_ach1, W_ach2, W_thal, Pi_mat, noise_mat, params_new);
    double ll_new = eval_distillation(bio_mu_new, rnn_mu);
