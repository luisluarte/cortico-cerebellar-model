...
cat("Starting 11_factorial_fixed_10k.R (Full Grid, Deterministic ODE + Covariance, 4 Chains, 10k iters)...\n")
...
for(s in 0:(length(subjs)-1)) {
  ...
  # Pre-generate deterministic noise matrix per subject
  noise_mats[[s+1]] <- matrix(rnorm(896 * length(s_idx)), nrow=896, ncol=length(s_idx))
}

sourceCpp("bio_mcmc_fixed_cov_full.cpp")
...
run_baseline_chain_adaptive <- function(eval_fn, N_params, s_idx, target_mode, iters=10000) {
  trace_z <- matrix(0, iters, N_params)
  trace_ll <- matrix(0, iters, length(s_idx))
  
  Z_curr <- rnorm(N_params, 0, 0.01)
  ...
  for(iter in 1:iters) {
    Z_new <- Z_curr + rnorm(N_params) * step_size
    ll_new <- eval_fn(Z_new, s_idx, target_mode)
    
    prior_old <- sum(dnorm(Z_curr, 0, 1, log=TRUE))
    prior_new <- sum(dnorm(Z_new, 0, 1, log=TRUE))
    
    log_alpha <- (ll_new + prior_new) - (ll_curr + prior_old)
    if(is.nan(log_alpha)) log_alpha <- -1e9
    
    accept_prob <- exp(log_alpha)
...
if(file.exists("../../results/factorial_fixed_10k_checkpoint.rds")) {
    all_results <- readRDS("../../results/factorial_fixed_10k_checkpoint.rds")
...
for(m in models) {
  ...
  res <- mclapply(1:nrow(tasks), function(i) {
    s <- tasks$subj_id[i]
    s_idx <- which(subj_indices == s)
    set.seed(42 + s) # VERY IMPORTANT: Seed strictly by subject so all chains see SAME deterministic noise!
    
    if(m$fn == "bio") {
      out <- run_bio_mcmc_fixed_cov_full(iters, X[s_idx,], ITI[s_idx], reward[s_idx], choices[s_idx], y_rt[s_idx], rnn_mu[s_idx,], m$target, m$prior, mu_EB, W_gen, W_ach1, W_ach2, W_thal, Pi_mat[s_idx,], noise_mats[[s+1]])
      return(out)
    } else if(m$fn == "q") {
      out <- run_baseline_chain_adaptive(eval_q, 3, s_idx, m$target, iters)
      return(out)
    } else if(m$fn == "w") {
      out <- run_baseline_chain_adaptive(eval_wsls, 2, s_idx, m$target, iters)
      return(out)
    }
  }, mc.cores=n_cores)
  ...
  # Optional: Store trace_z for the final results, averaged/subsampled to save RAM if needed.
  # We will just store the arrays to a lightweight structure.
  traces <- list()
  ...
  saveRDS(all_results, "../../results/factorial_fixed_10k_checkpoint.rds")
}

saveRDS(all_results, "../../results/factorial_fixed_10k_A.rds")
cat("Finished! Saved factorial_fixed_10k_A.rds\n")
