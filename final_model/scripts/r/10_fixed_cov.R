...
cat("Starting 10_fixed_cov.R (bio_dist, Deterministic ODE + Covariance Adaptation, 4 Chains)...\n")
...
sourceCpp("bio_mcmc_fixed_cov.cpp")
...
res <- mclapply(1:nrow(tasks), function(i) {
  s <- tasks$subj_id[i]
  s_idx <- which(subj_indices == s)
  # Set a different seed per chain so the fixed noise matrix is unique per chain
  set.seed(42 + s) 
  out <- run_bio_mcmc_fixed_cov(iters, X[s_idx,], ITI[s_idx], reward[s_idx], choices[s_idx], y_rt[s_idx], rnn_mu[s_idx,], "distillation", "locked", mu_EB, W_gen, W_ach1, W_ach2, W_thal, Pi_mat[s_idx,])
  return(out)
}, mc.cores=n_cores)
...
cat(sprintf("\n--- FINAL RESULTS (bio_dist with Deterministic ODE + Covariance) ---\nMin Rhat: %.3f | Median Rhat: %.3f | Mean Rhat: %.3f | Max Rhat: %.3f\nMin ESS: %.1f\n", 
