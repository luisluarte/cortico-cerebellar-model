...
sourceCpp("bio_mcmc_chain_adaptive.cpp")

n_cores <- 24
iters <- 1000
chains <- 3
burn <- 501:1000
tasks <- expand.grid(subj_id = 0:(length(subjs)-1), chain = 1:chains)

cat("Running Model: bio_anch (Adaptive MCMC, 1000 iters, 3 chains)...\n")
res <- mclapply(1:nrow(tasks), function(i) {
  s <- tasks$subj_id[i]
  s_idx <- which(subj_indices == s)
  out <- run_bio_mcmc_chain(iters, X[s_idx,], ITI[s_idx], reward[s_idx], choices[s_idx], y_rt[s_idx], rnn_mu[s_idx,], "empirical", "anchored", mu_EB, W_gen, W_ach1, W_ach2, W_thal, Pi_mat[s_idx,])
  return(out)
}, mc.cores=n_cores)
...
for(s in 0:(length(subjs)-1)) {
  ...
  arr <- array(0, dim=c(length(burn), chains, num_params))
  for(c in 1:chains) {
...
total_ll_trace <- matrix(0, length(burn)*chains, nrow(d))
trace_row <- 1
for(c in 1:chains) {
