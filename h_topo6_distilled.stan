  ...
  real subject_lpmf(int subj_seq, array[] int subj_all, array[] int Bd1_all, array[] int Bd2_all, array[] int Resp_all, array[] real Reward_all, array[] real RT_all, array[] real soft_p_all, array[] int start_idx, array[] int num_trials, real alpha_Q, real alpha_PC, real phys_decay, real g_s, real beta_v, real a_base, real ndt_base, real ndt_scale, vector kappa_vec, matrix inv_W_exp, int N_granules) {
    ...
    for (t in 1:n_t) {
      ...
      if (RT_all[idx] > ndt_dyn) {
        if (Resp_all[idx] == 1) { lp += wiener_lpdf(RT_all[idx] | a_dyn, ndt_dyn, 0.5, v_t) - log_prob_c1; } 
        else { lp += wiener_lpdf(RT_all[idx] | a_dyn, ndt_dyn, 0.5, -v_t) - log_prob_c2; }
      } else {
...
model {
  ...
  for (s in 1:N_subj) {
    target += subject_lpmf(subj_seq[s] | subj, Bd1, Bd2, Resp, Reward, RT, soft_p, start_idx, num_trials,
...
generated quantities {
  real sum_log_lik = 0;
  for (s in 1:N_subj) {
    sum_log_lik += subject_lpmf(subj_seq[s] | subj, Bd1, Bd2, Resp, Reward, RT, soft_p, start_idx, num_trials,
                           alpha_Q[s], alpha_PC[s], phys_decay[s], g_s[s], beta_v[s], a_base[s], ndt_base[s], ndt_scale[s],
                           kappa_vec, inv_W_exp, N_granules);
  }
}
