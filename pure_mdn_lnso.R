...
# Filter for first 50 unique subjects
subjs_all <- unique(stan_data$subj)
subjs_50 <- subjs_all[1:50]
saveRDS(subjs_50, 'excluded_subjects.rds')
...
epochs <- 40 # Fast training for LNSO

cat("Starting LNSO Evaluation...\\n")
...
cat("Mean L_pure (MDN):", mean(L_pure), "\\n")
