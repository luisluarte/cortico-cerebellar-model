...
clusterEvalQ(cl, {
  ...
  Phase3HybridUniversalRNN <- nn_module(
    "Phase3HybridUniversalRNN",
    initialize = function(input_dim, hidden_dim, alpha, beta, D_compress, sigma_sq) {
    ...
    forward = function(x, iti_seq, h0 = NULL) {
      ...
      # Axiom 2 & 5: The Phantom Trace with Thermodynamic Diffusion
      ...
      for (t in 1:seq_len) {
        ...
        # Axiom 5: Diffuse Z_t based on ITI
        noise_std <- sqrt(self$sigma_sq * dt)
        if (self$training) {
          Z_t_noisy <- Z_t + torch_randn_like(Z_t) * noise_std
        } else {
          # In eval mode, we don't inject noise to preserve deterministic CRPS metric, 
          # or we could. Let's keep noise for biological realism during eval too?
          # Actually, if we want deterministic evaluations we should turn off noise.
          # Wait, biological noise is always present. We will keep it.
          Z_t_noisy <- Z_t + torch_randn_like(Z_t) * noise_std
        }
        
        # Axiom 2: Accumulate on Noisy Z
        Z_accum <- Z_tilde + self$alpha * Z_t_noisy * (1 - Z_tilde)
...
evaluate_population <- function(params_matrix) {
  write.table(params_matrix, "nsga2_phase3_checkpoint_pop.csv", append = TRUE, col.names = FALSE, row.names = FALSE, sep=",")
  ...
  results <- foreach(i = 1:nrow(params_matrix), .combine = rbind, .packages = c("torch")) %dopar% {
    ...
    sigma_sq <- params_matrix[i, 4]
    
    input_dim <- ncol(X)
    hidden_dim <- 32
    
    model <- Phase3HybridUniversalRNN(input_dim, hidden_dim, alpha, beta, D_compress, sigma_sq)
    # Global L2 Decay to prevent NaN overflow, lower learning rate (0.002)
    ...
    tryCatch({
      ...
      if (is.na(reliance) || is.nan(reliance)) reliance <- 0
      
      res_line <- data.frame(alpha = alpha, beta = beta, D_compress = D_compress, sigma_sq = sigma_sq, CRPS = mean_crps, Reliance = reliance)
      write.table(res_line, "nsga2_phase3_checkpoint_results.csv", append = TRUE, col.names = FALSE, row.names = FALSE, sep=",")
...
# 4. Execute NSGA-II Phase 3
cat("Launching 32-core parallel NSGA-II for Phase 3...\\n")
if (file.exists("nsga2_phase3_checkpoint_pop.csv")) file.remove("nsga2_phase3_checkpoint_pop.csv")
if (file.exists("nsga2_phase3_checkpoint_results.csv")) file.remove("nsga2_phase3_checkpoint_results.csv")

res <- nsga2(evaluate_population, idim = 4, odim = 2, 
             lower.bounds = c(0.01, 0.001, 16, 0.0001), 
             upper.bounds = c(1.0, 0.5, 400, 0.1), 
             popsize = 32, generations = 15, vectorized = TRUE)

stopCluster(cl)
saveRDS(res, 'nsga2_phase3_crps_final.rds')
cat("PHASE 3 EVOLUTION COMPLETE.\\n")
