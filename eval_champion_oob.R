...
HybridUniversalRNN <- nn_module(
  ...
  initialize = function(input_dim, hidden_dim, E_ratio, p) {
    ...
    set.seed(42) # Reproducible Achlioptas matrix
...
E_opt <- 28
p_opt <- 0.287
model <- HybridUniversalRNN(input_dim, hidden_dim, E_opt, p_opt)
optimizer <- optim_adam(model$parameters, lr = 0.01)

epochs <- 40
cat("Training Champion Surrogate (E=28, p=0.287) on Train Subjs (N=50)...\\n")
...
cat("Evaluating Champion Surrogate on Out-Of-Bag Subjs (N=50)...\\n")
...
cat(sprintf("\\n=== Champion Hybrid Surrogate (E=28, p=0.287) OOB Metrics (N=50) ===\\n"))
